...
class DynamicDriverInstaller:
    """Checks missing printer driver packages and installs them on-demand via pkexec."""
    ...
    def check_and_install_driver(cls, parent_window, make_and_model: str) -> bool:
        """Prompts the user via GTK3 dialog on main thread and installs missing package safely."""
        pkg_name = cls.get_required_package(make_and_model)
        
        # Package not needed or already installed
        if not pkg_name or cls.is_package_installed(pkg_name):
            return True

        # Ask user via GTK3 dialog (All strings in English)
        ...
        if response == Gtk.ResponseType.YES:
            # Install securely using Polkit password prompt
            cmd = ["pkexec", "apt-get", "install", "-y", pkg_name]
            try:
                logger.info(f"Installing missing driver package '{pkg_name}' via pkexec...")
